...
USERS=123456789,987654321,112233445    # Список разрешенных пользователей

# Остальные параметры уже заполнены корректными значениями
```
...
- `/reboot` или кнопка "🔄 Перезагрузить сервер" - перезагрузка Ubuntu сервера

⚠️ **Внимание**: Кнопка перезагрузки сервера выполняет полную перезагрузку Ubuntu. Бот автоматически запустится после перезагрузки (автозапуск включен).
